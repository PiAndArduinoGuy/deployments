...
3. Next open a terminal on your local machine and *cd* to this project, execute *ansible-playbook deploy_and_start_control_hub_frontend.yml -i ../hosts.ini*. This will run a number of Ansible tasks that:
    * Update the Raspberry Pi apt repositories and upgrade the OS.
    * Create necessary directories
    * Install Tomcat 9 server.
    * Deploy the frontend.
    * Create systemd unit files to manage the frontend.
    * Create a static IP address for the Raspberry Pi on your private network.
    * Restart your RaspberryPi and host the frontend.
...
2. Next open a terminal on your local machine and *cd* to this project, execute *ansible-playbook deploy_and_start_rabbitmq.yml -i ../hosts.ini*. This will run a number of Ansible tasks that:
   ...
   * Create necessary directories
   * Installs Docker
   * Creates a RabbitMQ Docker container and starts the RabbitMQ server
   * Create systemd unit files to manage the RabbitMQ Docker container.
   * Create a static IP address for the Raspberry Pi on your private network.
   * Restarts your RaspberryPi and hosts the RabbitMQ server.
...
2. Next open a terminal on your local machine and *cd* to this project, execute *ansible-playbook deploy_and_start_security_micro_service.yml -i ../hosts.ini*. This will run a number of Ansible tasks that:
   * Update the Raspberry Pi apt repositories and upgrade the OS.
   * Create necessary directories
   * Installs OpenCV, extracts the yolo object detector script and saves a default security config file to the Raspberry Pi
   * Deploy the security micro-service jar artifact.
   * Create systemd unit files to manage the frontend.
   * Create a static IP address for the Raspberry Pi on your private network.
   * Restart your RaspberryPi and host the security micro-service.
...
2. Insert the SD card into your RaspberryPi and power it on.
3. Give the RaspberryPi time to connect to your private network, then find the IP address of your RaspberryPi by either logging into your router and opening the *Devices* tab or a similar option (depending on your router's make and model). Alternatively use a network utility like [netstat](https://linux.die.net/man/8/netstat) to determine the IP address of your RaspberryPi.
4. Replace the IP address of the *[security_camera_microservice]* host with the IP address obtained in step 3 in the hosts.ini file.
5. Next open a terminal on your local machine and *cd* to this project, execute *ansible-playbook deploy_and_start_security_camera_micro_service.yml -i ../hosts.ini*. This will run a number of Ansible tasks that:
   * Update the Raspberry Pi apt repositories and upgrade the OS.
   * Create necessary directories
   * Installs Python3 and the required python packages to run the security_camera_microservice python project.
   * Deploy the security_camera_microservice python artifact.
   * Creates a systemd unit file to manage the security_camera_microservice.
   * Create a static IP address for the Raspberry Pi on your private network.
   * Restart your RaspberryPi and host the security camera micro-service.
6. If you have a 3D printer then you can print the enclosure designed for the security camera, the STL files are available in the directory artifacts/other/enclosures 
...
2. Insert the SD card into your RaspberryPi and power it on.
3. Give the RaspberryPi time to connect to your private network, then find the IP address of your RaspberryPi by either logging into your router and opening the *Devices* tab or a similar option (depending on your router's make and model). Alternatively use a network utility like [netstat](https://linux.die.net/man/8/netstat) to determine the IP address of your RaspberryPi.
4. Replace the IP address of the *[alerter]* and *[hardware-security-controller]* host with the IP address obtained in step 3 in the hosts.ini file.
5. Next open a terminal on your local machine and *cd* to this project, execute *ansible-playbook deploy_and_start_hardware_security_controller.yml -i ../hosts.ini*. This will run a number of Ansible tasks that:
   * Update the Raspberry Pi apt repositories and upgrade the OS.
   * Create necessary directories
   * Installs Python3 and the required python packages to run the hardware_security_controller python project.
   * Deploy the hardware_security_controller python artifact.
   * Creates a systemd unit file to manage the hardware_security_controller.
   * Create a static IP address for the Raspberry Pi on your private network.
   * Restart your RaspberryPi and host the hardware_security_controller.
6. Execute *ansible-playbook deploy_and_start_alerter.yml -i ../hosts.ini*. This will run a number of Ansible tasks that:
   * Update the Raspberry Pi apt repositories and upgrade the OS.
   * Create necessary directories
   * Installs Python3 and the required python packages to run the alerter python project.
   * Deploy the alerter python artifact.
   * Creates a systemd unit file to manage the alerter.
   * Create a static IP address for the Raspberry Pi on your private network.
   * Restart your RaspberryPi and host the alerter.
7. If you have a 3D printer then you can print the enclosure designed for the hardware controller and alerter, the STL files are available in the directory artifacts/other/enclosures
